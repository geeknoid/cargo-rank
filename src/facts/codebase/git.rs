...
async fn get_repo_core(repo_path: &Path, repo_url: &Url) -> Result<()> {
    ...
    let output = run_git_with_timeout(&["-C", path_str, "fetch", "origin", "--depth=1", "--prune", "--force"]).await?;
...
async fn clone_repo(repo_path: &str, repo_url: &Url) -> Result<()> {
    log::info!(target: LOG_TARGET, "Syncing repository '{repo_url}'");
    let output = run_git_with_timeout(&["clone", "--depth=1", "--single-branch", "--no-tags", repo_url.as_str(), repo_path]).await?;
