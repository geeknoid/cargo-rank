...
impl Provider {
    ...
    pub async fn get_codebase_data(
        ...
        tracker.add_many_requests("repos", repo_crates.len() as u64);
        ...
        let (cached_results, needs_repo_fetch) = spawn_blocking(move || {
            ...
            let mut needs_repo_fetch: HashMap<Arc<RepoSpec>, Vec<CrateSpec>> = HashMap::new();
            ...
            for (repo_spec, crates) in repo_crates {
                ...
                for crate_spec in &crates {
                    ...
                    if let Some(cached_data) = cache_doc::load_with_ttl(
                        ...
                        format!("codebase data for crate '{crate_name}'"),
                    ) {
                ...
                if any_missing {
                    ...
                    for (crate_spec, cached_data) in all_cached_data {
                        ...
                        tracker_for_blocking.complete_request("repos");
                    }
                }
            }
        ...
        cached_results.into_iter().chain(
    ...
    async fn fetch_repo_data(self, repo: Arc<RepoSpec>, tracker: RequestTracker) -> (Arc<RepoSpec>, Result<RepoData>) {
        let result = self.fetch_repo_data_core(&repo).await;
        tracker.complete_request("repos");

        (repo, result)
    }

    async fn fetch_repo_data_core(&self, repo: &RepoSpec) -> Result<RepoData> {
        let repo_path = self.get_repo_cache_path(repo);

        // Sync/update the repository
        let git_result = tokio::time::timeout(GIT_REPO_TIMEOUT, git::get_repo(&repo_path, repo.url())).await;
        ...
        log::debug!(target: LOG_TARGET, "Detecting workflows in repository '{repo}'");
        let repo_path_for_workflows = repo_path.clone();
        let workflows = match spawn_blocking(move || sniff_github_workflows(&repo_path_for_workflows))
            .await
            .expect("task must not panic")
        {
            Ok(info) => info,
        ...
        log::debug!(target: LOG_TARGET, "Analyzed repository '{repo}', found {} packages", metadata.packages.len());
    ...
    async fn analyze_crate(
        ...
        let Some(package) = repo_data.metadata.packages.iter().find(|p| p.name == crate_name) else {
            ...
            return (crate_spec, ProviderResult::CrateNotFound);
        };
        ...
        let result = spawn_blocking({
            let codebase_data = codebase_data.clone();
    ...
    fn get_repo_cache_path(&self, repo_spec: &RepoSpec) -> PathBuf {
        ...
        self.cache_dir.join("repos").join(safe_owner).join(safe_repo)
    }

    /// Get the codebase data file path for a specific crate in a repository
    ///
    /// Returns `cache_dir/analysis/owner/repo/crate_name.json`
