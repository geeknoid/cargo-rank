...
async fn try_branch(
    ...
    log::info!(target: LOG_TARGET, "Querying codecov.io for coverage of repository '{repo_spec}'");

    let response = client
        .get(&codecov_url)
        .send()
        .await
        .inspect_err(|e| log::debug!(target: LOG_TARGET, "Could not send HTTP request to codecov.io ('{branch}' branch): {e:#}"))?;
