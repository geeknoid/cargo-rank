...
pub fn calculate_docs_metrics(reader: impl Read, crate_spec: &CrateSpec) -> Result<DocsData> {
    ...
    let state = match format_version {
        50 => calculate_metrics_v50(json_value, crate_spec).map(MetricState::Found)?,
        51 => calculate_metrics_v51(json_value, crate_spec).map(MetricState::Found)?,
        52 => calculate_metrics_v52(json_value, crate_spec).map(MetricState::Found)?,
        53 => calculate_metrics_v53(json_value, crate_spec).map(MetricState::Found)?,
        54 => calculate_metrics_v54(json_value, crate_spec).map(MetricState::Found)?,
        55 => calculate_metrics_v55(json_value, crate_spec).map(MetricState::Found)?,
        56 => calculate_metrics_v56(json_value, crate_spec).map(MetricState::Found)?,
        57 => calculate_metrics_v57(json_value, crate_spec).map(MetricState::Found)?,
        _ => {
            log::debug!(target: LOG_TARGET, "Unsupported rustdoc JSON format version {format_version} for {crate_spec}");
            MetricState::UnknownFormatVersion(format_version)
        }
    };
    ...
    Item: ItemLike,
{
    ...
    #[expect(clippy::cast_possible_truncation, reason = "percentage is always 0-100")]
    #[expect(clippy::cast_sign_loss, reason = "round() result is always positive")]
    ...
    let doc_coverage_percentage = if number_of_public_api_elements > 0 {
        (documented_count as f64 / number_of_public_api_elements as f64 * 100.0).round() as u8
    } else {
        0
    };
    ...
    let metrics = DocsMetrics {
        ...
        number_of_public_api_elements,
        number_of_undocumented_elements: number_of_public_api_elements - documented_count,
        number_of_examples_in_docs: number_of_examples_in_docs as u64,
