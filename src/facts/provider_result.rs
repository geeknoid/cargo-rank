...
impl<T: Clone> ProviderResult<T> {
    ...
    pub fn into_result(self) -> crate::Result<T> {
        match self {
            ...
            Self::CrateNotFound => ohno::bail!("crate not found"),
            Self::VersionNotFound => ohno::bail!("version not found"),
            Self::Error(e) => Err(Arc::try_unwrap(e).unwrap_or_else(|arc_e| ohno::app_err!("{arc_e:#}"))),
        }
    }
