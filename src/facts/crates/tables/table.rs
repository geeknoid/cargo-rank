...
pub trait Table: Sized {
    ...
    fn open_with(mmap: Mmap, max_ttl: Duration) -> Result<Self>;
    ...
    fn create_table(tables_root: impl AsRef<Path>, csv_entry: impl IoRead) -> Result<File> {
...
pub fn validate_table_header(mmap: &Mmap, max_ttl: Duration) -> Result<(u64, DateTime<Utc>)> {
    ...
    let age_seconds = now.saturating_sub(table_timestamp);
