...
impl Provider {
    ...
    pub async fn get_crate_data(
        ...
        let message = if crates.len() == 1 {
            format!("crate '{}'", crates[0].name())
        } else {
            format!("{} crates", crates.len())
        };
        let message = Arc::new(message);
        progress.configure_indeterminate(move || (*message).clone());

        tokio::task::spawn_blocking(move || provider.collect_crate_data(crates))
            .await
    ...
    fn assemble_query_result(
        ...
        let crate_spec = if let Some(repo_url) = &overall_data.repository {
        ...
        (crate_spec, ProviderResult::Found((version_data, overall_data)))
    }
