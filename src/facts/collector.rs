...
impl Collector {
    ...
    pub async fn collect(&self, crate_refs: impl IntoIterator<Item = CrateRef>) -> Result<impl Iterator<Item = (CrateSpec, CrateFacts)>> {
        ...
        self.progress.set_phase("Analyzing");
        let crate_data = self.crates_provider.get_crate_data(missing_refs, &self.progress).await;
        ...
        let crate_data_vec: Vec<_> = {
            ...
            let unique_crates: HashMap<CrateSpec, ProviderResult<(CrateVersionData, CrateOverallData)>> =
                crate_data.fold(HashMap::new(), |mut map, (crate_spec, provider_result)| {
        ...
        // Step 5: Collection phase - parallel data gathering
    ...
    fn load_cached_facts(&self, crate_refs: impl IntoIterator<Item = CrateRef>, min_time: DateTime<Utc>) -> Result<LoadCacheResult> {
        ...
        for crate_ref in crate_refs {
            let Some(crate_spec) = crate_ref.to_spec() else {
                missing_refs.push(crate_ref);
                continue;
            };

            if let Some(facts) = self.load_from_cache(&crate_spec, min_time)? {
                cached_facts.push((crate_spec, facts));
                continue;
            }
    ...
    async fn query_providers(
        ...
        let mut facts_map: HashMap<CrateSpec, CrateFacts> = crate_datas
            .into_iter()
            .map(|(crate_spec, provider_result)| {
        ...
        if !all_queryable_specs.is_empty() {
            ...
            for (crate_spec, facts) in &facts_map {
                if facts.is_complete() {
                    log::debug!(target: LOG_TARGET, "Facts are complete for {crate_spec}, saving to cache");
                    let cache_path = get_cache_path(&self.facts_cache_dir, crate_spec);
                    if let Err(e) = cache_doc::save(facts, &cache_path) {
                        log::debug!(target: LOG_TARGET, "Could not cache facts for {crate_spec}: {e:#}");
                    } else {
                        log::debug!(target: LOG_TARGET, "Successfully cached facts for {crate_spec}");
                    }
                } else {
                    log::debug!(target: LOG_TARGET, "Facts are incomplete for {crate_spec}, not caching. Status: crate_version={}, crate_overall={}, hosting={}, advisory={}, codebase={}, coverage={}, docs={}",
